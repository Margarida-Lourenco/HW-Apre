...
\begin{enumerate}
    ...
    \vspace{10pt}
    Comparing the obtained in the OLS and Ridge regression models, we can see that in the Ridge regression model $w_0$ is smaller and $w_1$ is larger than the corresponding weights in the OLS model.

    \vspace{10pt}
    To check the impact of the regularization term on the weights, we can calculate the corresponding normalized values:
...
\vspace{10pt}
When analizing MAE values (graphics below), the smaller the MAE the better the models performance. With that being said, the best combination for each heatmap is:
